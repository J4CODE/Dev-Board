...
const Board = ({ match }) => {
  ...
  useEffect(() => {
    if (board?.title) document.title = board.title + ' | Dev Board';
  }, [board?.title]);
    ...
    <div
      ...
      style={{
        backgroundImage:
          ...
          (board.backgroundURL
            ? board.backgroundURL
            : 'https://images.unsplash.com/photo-1505996000701-debaf9186e2d?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1172&q=80') +
          ')',
      }}
    >
      <Navbar />
